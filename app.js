...
const embeddedDictionaries = {
  ...
  en: {
    ...
    legal: { kicker: 'Legal', title: 'Imprint, privacy and cookies', subline: 'Short, transparent, and without tracking noise.', privacyTitle: 'Privacy', privacyText: 'No forms, analytics or advertising trackers. The host only processes technically necessary access data.', cookieTitle: 'Cookies', cookieText: 'No tracking cookies. Only the selected language is stored locally.', externalTitle: 'External content', externalText: 'Project images, certificates and the GitHub graphic are loaded locally. External links only open after a click.', imprintTitle: 'Imprint', imprintText: 'Private portfolio website by Aleksandar Nikolić. Contact is available in the footer.', contactLink: 'Contact me', unifiedTitle: 'Imprint and legal information', unifiedText: 'One shared explanation of the imprint, privacy, cookies and hosting.' },
    portfolio: {
      kicker: 'Projects',
      title: 'Selected work',
...
const renderOpenSourceContributions = () => {
  ...
  featured.forEach((contribution) => {
    ...
    const contributionKind = item.kind === 'issue' ? 'Issue' : 'Pull Request';
    card.setAttribute('aria-label', `${item.repo} ${contributionKind} ${item.number}: ${item.title}`);
