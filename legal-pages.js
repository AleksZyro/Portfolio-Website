...
const languageButton = document.getElementById('legal-language-button');
const languageMenu = document.getElementById('legal-language-menu');
const languageFlag = document.getElementById('legal-language-flag');
const languageLabel = document.getElementById('legal-language-label');
const languageFromUrl = new URLSearchParams(window.location.search).get('lang');
const languageLabels = { de: 'DE', en: 'EN', fr: 'FR', sr: 'SR (lat)', 'sr-cyrl': 'SR (kyr)' };
...
const legalCopy = {
  de: {
    ...
    impressum: { title: 'Impressum', sections: [
      ...
      section('Kontakt', 'Für Fragen zu Projekten oder einer Praktikumsstelle bin ich per E-Mail erreichbar: aleksandar09n@gmail.com.'),
  ...
  en: {
    ...
    impressum: { title: 'Imprint', sections: [
      ...
      section('Contact', 'For questions about projects or an internship, I can be reached by email at aleksandar09n@gmail.com.'),
  ...
  fr: {
    ...
    impressum: { title: 'Mentions légales', sections: [
      ...
      section('Contact', 'Pour toute question sur les projets ou un stage, je suis joignable par e-mail à aleksandar09n@gmail.com.'),
  ...
  sr: {
    ...
    impressum: { title: 'Impresum', sections: [
      ...
      section('Kontakt', 'Za pitanja o projektima ili praksi dostupan sam putem e-maila: aleksandar09n@gmail.com.'),
  ...
  'sr-cyrl': {
    ...
    impressum: { title: 'Импресум', sections: [
      ...
      section('Контакт', 'За питања о пројектима или пракси доступан сам путем имејла: aleksandar09n@gmail.com.'),
...
const unifiedLegalCopy = {
  de: {
    ...
    sections: [
      section('Impressum und Kontakt', 'Diese Website ist eine private Portfolio-Seite von Aleksandar Nikolić. Sie präsentiert Ausbildung, Projekte, Zertifikate und berufliche Interessen. Für Fragen zu Projekten oder einer Praktikumsstelle bin ich per E-Mail unter aleksandar09n@gmail.com erreichbar.'),
      ...
      section('Verantwortung und Datenschutzrechte', 'Verantwortlich für diese Website und die beschriebenen Datenbearbeitungen ist Aleksandar Nikolić. Fragen zur Datenbearbeitung sowie Auskunfts-, Berichtigungs- oder Löschbegehren können per E-Mail an aleksandar09n@gmail.com gerichtet werden. Soweit technische Zugriffsdaten beim Hosting anfallen, richten sich deren Aufbewahrung nach den Einstellungen und Vorgaben des Hosting-Anbieters.')
    ]
  },
  en: {
    ...
    sections: [
      section('Imprint and contact', 'This is Aleksandar Nikolić\'s private portfolio website. It presents education, projects, certificates and professional interests. For questions about projects or an internship, I can be reached by email at aleksandar09n@gmail.com.'),
      ...
      section('Responsibility and privacy rights', 'Aleksandar Nikolić is responsible for this website and the data processing described here. Questions about data processing and requests for access, correction or deletion can be sent by email to aleksandar09n@gmail.com. Where technical access data is generated by the hosting service, its retention is governed by the hosting provider’s settings and requirements.')
    ]
  },
  fr: {
    ...
    sections: [
      section('Mentions légales et contact', 'Ce site est le portfolio privé d’Aleksandar Nikolić. Il présente sa formation, ses projets, ses certificats et ses intérêts professionnels. Pour toute question sur les projets ou un stage, je suis joignable par e-mail à aleksandar09n@gmail.com.'),
      ...
      section('Responsabilité et droits relatifs aux données', 'Aleksandar Nikolić est responsable de ce site et des traitements décrits ici. Les questions sur le traitement des données ainsi que les demandes d’accès, de rectification ou de suppression peuvent être adressées par e-mail à aleksandar09n@gmail.com. Lorsque des données techniques d’accès sont générées par l’hébergement, leur durée de conservation dépend des réglages et des exigences de l’hébergeur.')
    ]
  },
  sr: {
    ...
    sections: [
      section('Impresum i kontakt', 'Ovo je privatni portfolio sajt Aleksandra Nikolića. Predstavlja obrazovanje, projekte, sertifikate i profesionalna interesovanja. Za pitanja o projektima ili praksi dostupan sam putem e-maila: aleksandar09n@gmail.com.'),
      ...
      section('Odgovornost i prava na privatnost', 'Aleksandar Nikolić je odgovoran za ovaj sajt i ovde opisanu obradu podataka. Pitanja o obradi podataka, kao i zahtevi za pristup, ispravku ili brisanje, mogu se poslati e-mailom na aleksandar09n@gmail.com. Ako hosting generiše tehničke pristupne podatke, njihovo čuvanje zavisi od podešavanja i zahteva hosting provajdera.')
    ]
  },
  'sr-cyrl': {
    ...
    sections: [
      section('Импресум и контакт', 'Ово је приватни портфолио сајт Александра Николића. Представља образовање, пројекте, сертификате и професионална интересовања. За питања о пројектима или пракси доступан сам путем имејла: aleksandar09n@gmail.com.'),
      ...
      section('Одговорност и права на приватност', 'Александар Николић је одговоран за овај сајт и овде описану обраду података. Питања о обради података, као и захтеви за приступ, исправку или брисање, могу се послати имејлом на aleksandar09n@gmail.com. Ако хостинг генерише техничке приступне податке, њихово чување зависи од подешавања и захтева хостинг провајдера.')
    ]
  }
};
...
const applyLegalLanguage = (code) => {
  ...
  if (languageLabel) languageLabel.textContent = languageLabels[code] || languageLabels.de;
  if (languageFlag) languageFlag.className = `flag ${code === 'en' ? 'flag-gb' : code === 'fr' ? 'flag-fr' : code === 'sr' || code === 'sr-cyrl' ? 'flag-rs' : 'flag-de'}`;
...
if (languageButton && languageMenu) {
  ...
  languageMenu.querySelectorAll('[data-lang]').forEach((option) => {
    option.addEventListener('click', () => {
      ...
      applyLegalLanguage(option.dataset.lang);
    });
  });
  document.addEventListener('click', (event) => {
    if (!event.target.closest('.legal-language-switch')) {
      languageMenu.hidden = true;
      languageButton.setAttribute('aria-expanded', 'false');
    }
  });
